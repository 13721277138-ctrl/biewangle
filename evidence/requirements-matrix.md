...
2026-08-31 在 Node.js 24.19.0、pnpm 11.19.0 本地工作树实际执行：
...
pnpm miniprogram:verify         PASS（11 pages, 13 templates, 233138 bytes）
pnpm e2e                        PASS（8 / 8）
```
...
| Requirement | PWA | 微信 | 自动测试 / 合同 | 真实终端证据 | 状态 |
|---|---|---|---|---|---|
| B-001 手机单手核心检查 | 375/390 px 无横向溢出，触控目标与底部导航通过 | 原生短路径首页→Run，模拟器逐页操作 | responsive E2E、PWA vertical slice、微信 full-v1 | Chrome 手机视口与微信官方模拟器已验证；物理手机完整 Run 尚无人工记录 | `partial-external` |
...
| B-003 微信原生核心体验 | 不适用 | 11 个原生页面、无 `web-view`、13 模板完整链 | miniprogram verifier、微信 full-v1、native conformance | 官方逐页编译、模拟器业务闭环、手机自动预览推送、测试号 `1.1.0` 体验构建上传 | `partial-external`（物理手机完整走查待用户） |
...
| 微信开发者工具 | 11 页官方编译、真实模拟器业务闭环 | `verified` | `evidence/wechat/g5-full-v1-2026-08-30.md` |
| 微信手机预览 | 自动预览已推送到当前开发者微信，未伪造手机完整走查 | `partial-external` | `evidence/wechat/g5-semantic-audit-2026-08-31.md` |
| 微信体验构建 | 测试号 `1.1.0` 已上传并在语义审计后重新上传 | `verified` | 同上 |
